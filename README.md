...
- **Verified against a live Umbrella account.** Using a real API Key + Key Secret for a Managed Provider (MSSP) account: `cisco_umbrella_list_customers` returned 77 real customer organizations (real company names), and `cisco_umbrella_get_providers_console` returned a real subscription summary (`customerCount: 77`, matching the customer list — cross-validated). Both confirm the client_credentials auth exchange and request plumbing work end-to-end.
- **The 4 activity report tools + `cisco_umbrella_list_roaming_computers` returned well-formed but empty results** (`{"meta": {}, "data": []}` / `[]`) against this same account. This is very likely because the credential is a **Managed Provider root org**, which has no direct DNS/proxy/firewall/AMP traffic or roaming computers of its own — that data lives under each of the 77 *managed customer* orgs individually, not the provider root. None of Cisco's documented parameters for these endpoints include an explicit "query as this customer org" scoping option, so it's unclear from the docs alone how a provider-level credential is meant to pull a specific customer's activity data (may require a separate delegated-auth step, or MSPbots' actual usage may only ever call these against single-customer-level credentials, not provider-level ones). Needs a real single-customer-org credential (as opposed to a provider/MSSP credential) to fully verify these 5 tools return non-empty data.
- **`cisco_umbrella_list_applications`, `cisco_umbrella_list_protocols`, and `cisco_umbrella_list_application_categories` (App Discovery) all returned `403 Access Forbidden`** on this test account (reproduced identically via both the MCP tool and a direct curl call with the same token, ruling out a request-construction bug) — despite the token's scope list including `reports.appdiscovery:read`. Most likely this Umbrella package tier ("Umbrella for MSSPs" per the provider console response) doesn't include the App Discovery add-on/entitlement. Not a code issue; needs an account with App Discovery licensed to confirm.
- `cisco_umbrella_get_providers_console` returns a single subscription-summary object, not a list — despite its plural-sounding name in MSPbots' own configured API list ("Providers Consoles"). Confirmed via the live test above — worth double-checking against whatever MSPbots' existing collector expects (array vs single object).
...
- Scope is limited to the 10 operations MSPbots currently uses (user-confirmed), not Umbrella's full API surface (which also includes Networks, Internal Domains, Sites, Network Tunnels, Policies, Tagging, and the separate Key Admin API for managing API keys themselves).
