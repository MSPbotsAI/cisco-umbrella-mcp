...
- **⚠️ Not yet tested against a live Umbrella account.** All 10 tools + the auth exchange have been checked structurally (MCP handshake, tools-list, schema validity, `/health`, gateway 401 credential-gating). Endpoint paths/params were verified against the raw OpenAPI spec embedded in Cisco's own official developer docs (not summarized/guessed) — see source links in each tool's docstring area of the commit history — but no real API Key/Key Secret has been available yet to exercise a live call.
- `cisco_umbrella_get_providers_console` returns a single subscription-summary object, not a list — despite its plural-sounding name in MSPbots' own configured API list ("Providers Consoles"). Worth double-checking against whatever MSPbots' existing collector expects (array vs single object) once this is deployed.
