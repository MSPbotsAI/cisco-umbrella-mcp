...
from cisco_umbrella_mcp.__main__ import _build_http_app
from cisco_umbrella_mcp.config import Settings
from cisco_umbrella_mcp.server import create_mcp_server, get_client_from_context
...
def test_missing_header_returns_401_with_required_headers_listed():
    ...
    with TestClient(app) as client:
        ...
        body = resp.json()
        assert "X-Umbrella-Api-Key" in body["required_headers"]
        assert "X-Umbrella-Key-Secret" in body["required_headers"]


def test_only_one_of_two_required_headers_still_401():
    app, _ = _make_app()
    with TestClient(app) as client:
        resp = client.post(
            "/mcp",
...
def test_header_present_reaches_request_context(monkeypatch):
    ...
    # request are exactly what get_client_from_context sees, and that they're
    # reset afterward (no leakage to the next request).
    import asyncio

    from cisco_umbrella_mcp.server import GatewayTokenMiddleware, _gateway_creds_var
    ...
    async def run():
        scope = {
            ...
            "path": "/mcp",
            "headers": [
                (b"x-umbrella-api-key", b"test-key-123"),
                (b"x-umbrella-key-secret", b"test-secret-456"),
            ],
        }
    ...
    asyncio.run(run())
    assert seen["creds"] == ("test-key-123", "test-secret-456")
